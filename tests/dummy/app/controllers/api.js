...
export default Controller.extend({
	...
	actions: {
		loadPersonGroups: function() {
			var self = this;
			this.store.findAll('personGroup')
		...
		identify: function(){
			var self = this;
			var identifyRequest = this.store.createRecord('identifyRequest', {personGroupId: this.get('model.personGroup.id'), confidenceThreshold: .5, faceIds :[this.get('model.detectResponse').objectAt(0).get('id')] });
